// import { executeQuery } from '../lib/tursoClient'; // Migrado a MongoDB
...
export async function getAllDocumentos() {
  ...
  try {
    ...
    // Usar fetch con la API REST de MongoDB
    ...
    // Guardar en caché
    cacheService.set(DOCUMENTOS_CACHE_KEY, documentos, 300); // 5 minutos
...
export async function getDocumentoById(id) {
  try {
    ...
    if (!response.ok) {
      console.error(`Error al obtener documento con ID ${id}:`, response.statusText);
      // Buscar en los datos de muestra
      const documento = MOCK_DOCUMENTOS.find(doc => doc.id === parseInt(id));
...
export async function getAllCategorias() {
  ...
  try {
    ...
    // Usar fetch con la API REST de MongoDB
    const response = await fetch('/api/categorias-documentos');
    ...
    // Guardar en caché
    cacheService.set(CATEGORIAS_CACHE_KEY, categorias, 600); // 10 minutos
